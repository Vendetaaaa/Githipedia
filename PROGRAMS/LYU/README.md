...
```bash
pip install license-year-updater
```

## Usage

```bash
update-license-year [--dir PATH] [--action auto|add|subtract] [--all] [--dry-run]
```
...
update-license-year --dry-run

# Update all repos in ~/projects to current year
update-license-year --dir ~/projects --all

# Roll back one year
update-license-year --action subtract
```
...
```bash
pip uninstall license-year-updater
```
